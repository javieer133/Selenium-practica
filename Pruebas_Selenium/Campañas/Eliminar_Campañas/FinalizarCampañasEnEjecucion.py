...
class TerminarDeCrearCampanas(unittest.TestCase):
    ...
    def test_terminar_de_crear_campanas(self):
        ...
        while True:
            ...
            driver.find_element_by_xpath("//ul[@id='menu1']/li[2]/a/span[2]").click()
            try:
                ...
                if(current_campaign == "Crear nueva campaña"):
                    print("La lista de campañas esta vacia, no hay nada por finalizar...")
                    break            
            except:

                print("No se puede continuar finalizando campañas en ejecucion, posiblemente ya no quedan...")
                break

            if(current_campaign.find(self.skip) == -1):


                print("Finalizando campaña en ejecucion --->" + current_campaign)
                ...
                hover.perform()   
                driver.find_element_by_css_selector("#finish_campaign > span:nth-child(2)").click()
                driver.find_element_by_id("alertify-ok").click()
                driver.find_element_by_css_selector("span.fa.fa-bullhorn").click()
                print("      Campaña {} finalizada con exito".format(current_campaign))
